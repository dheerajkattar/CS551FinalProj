#!/usr/bin/env python3
"""
Serverless (Cloud Run) Performance Analysis
...
fig.suptitle("SERVERLESS (CLOUD RUN) - CRUD App Performance Analysis",
             fontsize=20, fontweight='bold', color='#3498db')

# Plot 1: Average Latency (MOST IMPORTANT)
ax1 = plt.subplot(2, 3, 1)
colors = ['#3498db', '#2ecc71', '#3498db', '#3498db', '#3498db', '#3498db']
...
for bar, val in zip(bars, avg_latency):
    ax1.text(bar.get_x() + bar.get_width()/2, val + 30, f'{val:.0f}ms',
...
ax2.fill_between(range(len(scenarios)), throughput, alpha=0.3, color='#3498db')
...
for i, val in enumerate(throughput):
    ax2.text(i, val + 1, f'{val:.1f}', ha='center', va='bottom', fontsize=8, fontweight='bold')

# Plot 3: P99 Latency (Tail Latency)
ax3 = plt.subplot(2, 3, 3)
bars = ax3.bar(range(len(scenarios)), p99_latency, color=['#2ecc71' if x < 2000 else '#3498db' for x in p99_latency],
...
for bar, val in zip(bars, p99_latency):
    ax3.text(bar.get_x() + bar.get_width()/2, val + 150, f'{val:.0f}ms',
            ha='center', va='bottom', fontsize=9, fontweight='bold')
...
bars = ax6.bar(range(len(scenarios)), tail_impact, color=['#e74c3c' if x > 1000 else '#f39c12' for x in tail_impact],
              alpha=0.8, edgecolor='black', linewidth=1.5)
...
plt.tight_layout()
plt.savefig("benchmarks/serverless_detailed_analysis.png", dpi=300, bbox_inches='tight')
print("✅ Serverless graph saved: benchmarks/serverless_detailed_analysis.png")

# Print summary
print("\n" + "="*80)
print("SERVERLESS (CLOUD RUN) - PERFORMANCE SUMMARY")
print("="*80)
print(f"\nRun ID: {data['run_id']}")
